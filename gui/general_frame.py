...
class GeneralFrame(Frame):
    def __init__(self, config, pluginmanager, master, *args, **kwargs):
        ...
        self.pluginPackSelectionBox = GenericComboBox(
            self.pluginPackControls, values=list(config["pluginPacks"].keys()), variable=self.loadedPluginPack
        )
        ...
        for pluginName, pluginInfos in self.pluginManager.get_available_plugins().items():
            ...
            self.pluginTable.insert(
                "",
                "end",
                value=(pluginName, pluginInfos.pluginDescription, pluginInfos.pluginVersion),
                tags="checked" if pluginInfos.isLoaded else "unchecked",
            )
